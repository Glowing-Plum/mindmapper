...
function syncScriptureSettings() {
  ...
  ui.verseHint.textContent = state.verseAction === 'jwlibrary'
    ? 'Opens JW Library through its own link. On an iPad or iPhone, Safari asks each time before opening the app.'
    : state.verseAction === 'jwshare'
      ? 'Opens the verse in JW Library, in the Bible it is set to. Without the app installed, it opens on jw.org.'
...
function boot() {
  ...
  state.verseAction = VERSE_ACTIONS.some((entry) => entry.id === talk.verseAction) ? talk.verseAction : 'jwshare';
